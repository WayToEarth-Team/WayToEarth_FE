...
import { NegativeAlert } from "@shared/ui/AlertDialog";
import type {
  Achievement,
  Summary,
  EmblemFilter,
...
const EmblemCollectionScreen: React.FC<{ navigation?: any }> = ({
  ...
  const fetchData = async () => {
    try {
      ...
      // Mark known 10m emblem as owned via code only; do not append local mock
      const dist10Owned = ownedCodes.includes("DIST_10M");
      const updated = (catalog || []).map((a) => {
        const is10m =
          typeof a?.name === "string" &&
          /10\s*m|10m|10 m|0\.01\s*km/i.test(a.name);
        return is10m && dist10Owned ? { ...a, owned: true } : a;
      });
      setAchievements(updated as any);
    } catch (e) {
      console.error(e);
      setAlert({
        open: true,
        title: "에러",
        message: "엠블럼 데이터를 불러오지 못했습니다.",
      });
    } finally {
      setLoading(false);
    }
